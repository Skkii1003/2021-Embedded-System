        ...
        if (OSIntNesting == 0u) {                          /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0u) {                     /* ... and not locked.                      */
                ...
                if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy */
                    ...
                    printf("\n%10d Preempt %d %d", OSTimeGet(), fromId, toId);
    ...
    if (OSIntNesting == 0u) {                          /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked                  */
            ...
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 2 - i; j++) {
                    ...
                    if (t2->Deadline < t1->Deadline) {
                        PrioChange = 1;
                        OSTaskChangePrio(t1->OSTCBPrio, 60);
                        OSTaskChangePrio(t2->OSTCBPrio, t1->OSTCBPrio);
                        OSTaskChangePrio(60, t2->OSTCBPrio);
                    }
                }
            }
            ...
            if (OSPrioHighRdy != OSPrioCur || PrioChange) {          /* No Ctx Sw if current task is highest rdy     */
                ...
                printf("\n%5d Complete %d %d", OSTimeGet(), fromId, toId);
            }
        }
    }
    OS_EXIT_CRITICAL();
